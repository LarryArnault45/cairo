...
The type `()` has only one possible value: the xref:literal-expressions.adoc[unit
literal] `()`.
...
[source,cairo]
----
let () = set_block_number(42);  // Explicitly match unit value
----
